...
<<<<<<< HEAD
		...
		super(operator, targets, evaluators, punishers, condition, content, state);
	}
	...
	public SPol evaluate(SCom sCom) {
		...
		if((this.getState().name().equals(States.ACTIVE.name()))
			...
			if(this.getContent().inconsistentContent(sCom.getContent())) {
				...
				sPol = new SPol(sCom, new ArrayList<SCom>(), new Long(
						System.currentTimeMillis()), new TrueCondition(),
						sCom.getContent(), SPol.States.VIOLATED, sCom.getFacet(),
						Dimensions.COMPETENCE, this.calculatePenalty(sCom.getContent()));
			} else {
				sPol = new SPol(sCom, new ArrayList<SCom>(), new Long(
						System.currentTimeMillis()), new TrueCondition(),
						sCom.getContent(), SPol.States.FULFILLED, sCom.getFacet(),
						Dimensions.COMPETENCE, this.calculatePenalty(sCom.getContent()));
			}
		}
	...
	private Float calculatePenalty(Object content) {
		if(content instanceof ARTContent) {
			...
			double nominator = ((Math.max(c.getOpinionValue().doubleValue(), c
					.getOfficialValue().doubleValue()) - Math.min(c.getOpinionValue()
					.doubleValue(), c.getOfficialValue().doubleValue())));
	...
	public Boolean match(SCom sCom) {
		...
		return new Boolean(this.getContent().getClass().getName()
				.equals(sCom.getContent().getClass().getName()));
=======
