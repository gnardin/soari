...
<<<<<<< HEAD
	 ...
	 * 
	 */
	public Vector<SPol> getSPolByCred(Reputation.RepType repType, String creditor) {
		Vector<SPol> result = new Vector<SPol>();
	 ...
	 * 
	 */
	private Map<String, ImpValue[]> Imp(Reputation.RepType repType, Facets facet,
					...
					for(SPol sPol : vSPol) {
						if(sPol != null) {
							if((sPol.getFacet().name().equals(facet.name()))
									&& (sPol.getDimension().name().equals(dimension.name()))) {
		...
		for(Facets facet : Facets.values()) {
			for(Dimensions dimension : Dimensions.values()) {
				...
				for(Iterator<String> debtors = imp.keySet().iterator(); debtors
					...
					// FULFILLED
					nominator = weight.getWeight(repType, Weight.WeightType.FULFILLED)
							* impValue[SPol.States.FULFILLED.ordinal()].getSumOfPenalties();
					...
					// VIOLATED
					nominator = nominator
							+ weight.getWeight(repType, Weight.WeightType.VIOLATED)
							* impValue[SPol.States.VIOLATED.ordinal()].getSumOfPenalties();
					
					denominator = denominator
							+ Math.abs(weight.getWeight(repType, Weight.WeightType.VIOLATED)
									.doubleValue())
							* impValue[SPol.States.VIOLATED.ordinal()].getSumOfPenalties();
					
					// CANCELED
					nominator = nominator
							+ weight.getWeight(repType, Weight.WeightType.CANCELLED)
							* impValue[SPol.States.CANCELLED.ordinal()].getSumOfPenalties();
					
					denominator = denominator
							+ Math.abs(weight.getWeight(repType, Weight.WeightType.CANCELLED)
									.doubleValue())
							* impValue[SPol.States.CANCELLED.ordinal()].getSumOfPenalties();
					...
					relevance = impValue[SPol.States.FULFILLED.ordinal()]
							.getNumberOfSPol()
							+ impValue[SPol.States.VIOLATED.ordinal()].getNumberOfSPol()
							+ impValue[SPol.States.CANCELLED.ordinal()].getNumberOfSPol();
					
					// Calculate
					if(denominator != 0) {
						value = new ReputationValue(new Double(nominator / denominator),
								relevance);
...
<<<<<<< HEAD
	...
	public void print(SPolSetType type) {
		...
		if((type == SPolSetType.DIbRpByFacet) || (type == SPolSetType.IIbRpByFacet)
...
<<<<<<< HEAD
		...
		if((type == SPolSetType.DIbRpByDeb) || (type == SPolSetType.IIbRpByDeb)
			...
			if(type == SPolSetType.DIbRpByDeb) {
				...
				System.out.println("=== Social Policy Set ObsRcb By Debitor ===");
				sPolSet = this.ObsRcbSPolSetByDeb;
			} else if(type == SPolSetType.EvRcbRpByDeb) {
				System.out.println("=== Social Policy Set EvRcb By Debitor ===");
				sPolSet = this.EvRcbSPolSetByDeb;
			}
...
<<<<<<< HEAD
		...
		if((type == SPolSetType.DIbRpByCred) || (type == SPolSetType.IIbRpByCred)
			...
			if(type == SPolSetType.DIbRpByCred) {
				...
				System.out.println("=== Social Policy Set ObsRcb By Creditor ===");
				sPolSet = this.ObsRcbSPolSetByCred;
			} else if(type == SPolSetType.EvRcbRpByCred) {
				System.out.println("=== Social Policy Set EvRcb By Creditor ===");
