...
public class RepageModule {
	...
	public boolean exists(String target, String facet) {
		...
		for(int i = 0; i < this.repage.size(); i++) {
			...
			if((unit.getTarget().toUpperCase().equals(target.toUpperCase()))
					&& (unit.getFacet().toUpperCase().equals(facet.toUpperCase()))) {
				exist = true;
				break;
			}
		}
	...
	public void calculateImage(String target, String facet, float newInput) {
		...
		try {
			...
			Predicate active = unit.getPredicate(Predicate.predicateType.FULFILLMENT);
			// strength = 0.8
			...
			aux.add(unit.getPredicate(Predicate.predicateType.IMAGE).getTuple());
			aux.add(unit.getPredicate(Predicate.predicateType.SHARED_EVAL).getTuple());
			aux.add(unit.getPredicate(Predicate.predicateType.OUTCOME).getTuple());
	...
	public void calculateImage(String target, String facet, String newInput) {
		this.calculateImage(target, facet, fromVectortoReal(newInput).floatValue());
	}
	...
	public void calculateTPImage(String target, String facet, float newInput) {
		...
		try {
			...
			// Level 1 of Repage's memory
			Predicate active = unit.getPredicate(Predicate.predicateType.TPIMAGE);
			// strength = 0.5
	...
	public void calculateReputation(String target, String facet, float newInput) {
		...
		try {
			RepageUnit unit = this.repage.get(getUnitIndex(target, facet));
			// Level 1 of Repage's memory
			Predicate active = unit.getPredicate(Predicate.predicateType.COM_REP);
			// strength = 0.3
			...
			aux.add(unit.getPredicate(Predicate.predicateType.REPUTATION).getTuple());
	...
	public String getDeviatedReputationText(String target, String facet,
		...
		String strReputation = convertRealToString(new Double(value).floatValue(),
				strength);
	...
	private String convertTuple(TupleFzz tuple) {
		...
		result = "[[" + decimalFormat.format(d[0]) + ";"
				...
				+ decimalFormat.format(d[3]) + ";" + decimalFormat.format(d[4]) + "];"
